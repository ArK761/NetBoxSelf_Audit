"""NetBoxSelf Audit: who changed what in NetBox itself (from the NetBox changelog).
...
recorded by the plugin (SelfAuditSystemEvent).
"""
...
from .common import SEVERITY_COLOR, SEVERITY_KEYS, SEVERITY_RANK, format_time as _format_time, severity_label
...
def check_system(settings, now: datetime | None = None) -> int:
    ...
    previous = getattr(settings, "system_snapshot", None) or {}
    if previous == current:
        return 0
    ...
    settings.system_snapshot = current
    settings.save(update_fields=["system_snapshot"])
    return len(events)
